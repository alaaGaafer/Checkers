...
def mode_sel():
    ...
    # mode
    mode_section = pygame.Rect(150, 200, screen_width - 280, 300)
    mode_title = font.render("Mode", True, BLACK)
    mode_title_rect = mode_title.get_rect(center=(mode_section.centerx, mode_section.top + 30))

    human_human_button = pygame.Rect(mode_section.left + 20, mode_section.top + 70, 20, 20)
    human_AI_button = pygame.Rect(mode_section.left + 20, mode_section.top + 140, 20, 20)
    computer_AI_button = pygame.Rect(mode_section.left + 20, mode_section.top + 210, 20, 20)

    # Create Play button
    play_button = pygame.Rect(mode_section.centerx - 50, mode_section.bottom + 30, 100, 50)
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if human_human_button.collidepoint(mouse_pos):
                    mode = "HH"
                elif human_AI_button.collidepoint(mouse_pos):
                    mode = "HAI"
                elif computer_AI_button.collidepoint(mouse_pos):
                    mode = "CAI"
        ...
        # Draw the mode section
        pygame.draw.rect(screen, BLACK, mode_section, 2)
        screen.blit(mode_title, mode_title_rect)
        pygame.draw.rect(screen, BROWN if mode == "HH" else BLACK, human_human_button,
                         width=2 if mode != "HH" else 0)
        pygame.draw.rect(screen, BROWN if mode == "HAI" else BLACK, human_AI_button,
                         width=2 if mode != "HAI" else 0)
        pygame.draw.rect(screen, BROWN if mode == "CAI" else BLACK, computer_AI_button,
                         width=2 if mode != "CAI" else 0)

        human_human_text = font.render("Human VS Human", True, BLACK)
        human_human_rect = human_human_text.get_rect()
        human_human_rect.midleft = (human_human_button.right + 10, human_human_button.centery)
        screen.blit(human_human_text, human_human_rect)

        human_AI_text = font.render("Human VS AI", True, BLACK)
        human_AI_rect = human_AI_text.get_rect()
        human_AI_rect.midleft = (human_AI_button.right + 10, human_AI_button.centery)
        screen.blit(human_AI_text, human_AI_rect)

        CAI_text = font.render("Computer VS AI", True, BLACK)
        CAI_rect = CAI_text.get_rect()
        CAI_rect.midleft = (computer_AI_button.right + 10, computer_AI_button.centery)
        screen.blit(CAI_text, CAI_rect)
...
def level_selection():
    ...
    while running:
        ...
        pygame.draw.rect(screen, BROWN if strategy == "Plot" else BLACK, plot_attack_button,
                         width=2 if strategy != "Plot" else 0)
        pygame.draw.rect(screen, BROWN if strategy == "Attack" else BLACK, attack_button,
                         width=2 if strategy != "Attack" else 0)
        ...
        pygame.draw.rect(screen, BROWN if algorithm == "Minimax" else BLACK, minimax_button,
                         width=2 if algorithm != "Minimax" else 0)
        pygame.draw.rect(screen, BROWN if algorithm == "Alpha" else BLACK, alpha_beta_button,
...
def start_gui(Board, leg, counter, player):
    Brown = (139, 69, 19)
    WHITE = (255, 233, 197)
    Blue = (255, 255, 0)
    DarkPieces = sum(row.count("DD") + row.count("sel") + row.count("selk") + row.count("DDK") for row in Board)
    WhitePieces = sum(row.count("DW") + row.count("sel") + row.count("selk") + row.count("DWK") for row in Board)
    ...
    screen_width = 725
    screen_height = 780  # Increased height to fill the bottom plank space
    screen = pygame.display.set_mode([screen_width, screen_height])
    ...
    DWKs = pygame.image.load('pics/whiteksel.png')
    DWs = pygame.image.load('pics/whitesel.png')
    ...
    DWKs = pygame.transform.scale(DWKs, (int(board_size / 8), int(board_size / 8)))
    DWs = pygame.transform.scale(DWs, (int(board_size / 8), int(board_size / 8)))
    ...
    for row in range(8):
        for col in range(8):
            ...
            elif board[row][col] == 'sel':
                board_surface.blit(DWs, (col * (board_size / 8), row * (board_size / 8)))
                board[row][col] = player
            elif board[row][col] == 'selk':
                board_surface.blit(DWKs, (col * (board_size / 8), row * (board_size / 8)))
                board[row][col] = player + 'K'

    for move in leg:
        row, col = move
        pygame.draw.circle(board_surface, Blue, (
            col * (board_size / 8) + 50, row * (board_size / 8) + 50), 15)
...
def updatee(borad, leg, player):
    if player == "DW":
        WhitePieces = sum(row.count("DW") + row.count("sel") + row.count("selk") + row.count("DWK") for row in board)
        DarkPieces = sum(row.count("DD") + row.count("DDK") for row in borad)

    elif player == "DD":
        WhitePieces = sum(row.count("DW") + row.count("DWK") for row in board)
        DarkPieces = sum(row.count("DD") + row.count("sel") + row.count("selk") + row.count("DDK") for row in borad)
    ...
    DWKs = pygame.image.load('pics/whiteksel.png')
    DWs = pygame.image.load('pics/whitesel.png')
    DW = pygame.transform.scale(DW, (int(725 / 8), int(725 / 8)))
    DD = pygame.transform.scale(DD, (int(725 / 8), int(725 / 8)))
    DDK = pygame.transform.scale(DDK, (int(725 / 8), int(725 / 8)))
    DWK = pygame.transform.scale(DWK, (int(725 / 8), int(725 / 8)))
    DWKs = pygame.transform.scale(DWKs, (int(725 / 8), int(725 / 8)))
    DWs = pygame.transform.scale(DWs, (int(725 / 8), int(725 / 8)))
    screen = pygame.display.set_mode([725, 780])
    screen.blit(header_image, (-5, -20))
    if DarkPieces < 10:
        screen.blit(darkcount, (number_pos_x - 106, number_pos_y - 30))
    else:
        screen.blit(darkcount, (number_pos_x - 115, number_pos_y - 30))
    if WhitePieces < 10:
        screen.blit(whitecount, (number_pos_x - 45, number_pos_y - 30))
    ...
    WHITE = (255, 233, 197)
    Blue = (255, 255, 0)
    board_surface = pygame.Surface([725, 725])
    for row in range(8):
        for col in range(8):
            if (row + col) % 2 == 0:
                color = WHITE
                ...
                color = Brown
            pygame.draw.rect(board_surface, color,
                             [col * (725 / 8), row * (725 / 8), (725 / 8), (725 / 8)])
    for row in range(8):
        for col in range(8):
            if board[row][col] == 'DW':
                board_surface.blit(DW, (col * (725 / 8), row * (725 / 8)))
            elif board[row][col] == 'DD':
                board_surface.blit(DD, (col * (725 / 8), row * (725 / 8)))
            elif board[row][col] == 'DDK':
                board_surface.blit(DDK, (col * (725 / 8), row * (725 / 8)))
            elif board[row][col] == 'DWK':
                board_surface.blit(DWK, (col * (725 / 8), row * (725 / 8)))
            elif board[row][col] == 'sel':
                board_surface.blit(DWs, (col * (725 / 8), row * (725 / 8)))
                board[row][col] = player
            elif board[row][col] == 'selk':
                board_surface.blit(DWKs, (col * (725 / 8), row * (725 / 8)))
                board[row][col] = player + 'K'
    for move in leg:
        row, col = move
        pygame.draw.circle(board_surface, Blue, (
            col * (725 / 8) + 50, row * (725 / 8) + 50), 15)
    screen.blit(board_surface, [((725 - 725) // 2), 55])
...
def play_alpha_beta_ai(board, player, depth, selected_strategy):
    value, best_move, selected_strategy = Alpha_beta_pruning(board, player, depth, -float('inf'), float('inf'), False,
                                                             selected_strategy)
    if best_move is not None:
        moveHuman(board, best_move[0], best_move[1], player, 1)
    return board
...
def winner(new_board, FromMain):
    ...
    if (WhitePieces == 0):
        if FromMain == 1:
            show_message_box("Winner is Black pieces, ate all the white pieces!")
        return True
    elif (DarkPieces == 0):
        if FromMain == 1:
            show_message_box("Winner is White pieces, ate all the Black pieces!")
        return True
    elif (not small_legal_moves(new_board, "DW")):
        if FromMain == 1:
            show_message_box("Winner is Dark pieces, no available moves for White pieces!")
        return True
    elif (not small_legal_moves(new_board, "DD")):
        if FromMain == 1:
            show_message_box("Winner is White pieces, no available moves for Dark pieces!")
        return True
    else:
        return False


def playhuman(board, player):
    selected_piece = None
    prev = ''
    prex = 0
    prey = 0
    ...
    while play == True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                mouse_pos = pygame.mouse.get_pos()
                # Calculate the cell position on the board
                cell_x = (mouse_pos[0] - board_x) // (board_size // 8)
                cell_y = (mouse_pos[1] - board_y) // (board_size // 8)
                # Check if the click is within the board boundaries
                if 0 <= cell_x < 8 and 0 <= cell_y < 8:
                    # Check if a piece is already selected
                    if selected_piece is not None and (
                            board[cell_y][cell_x] == 'W' or board[cell_y][cell_x] == 'D') and (cell_y, cell_x) in legal:
                        # Clear the selection border of the previously selected piece
                        board = moveHuman(board, (prey, prex), (cell_y, cell_x), player, 1)
                        play = False
                        break
                    else:
                        if selected_piece is not None:
                            board[prey][prex] = prev
                    # Check if the clicked cell has a piece
                    if board[cell_y][cell_x] != 'D' and board[cell_y][cell_x] != 'W':
                        selected_piece = 1
                        prev = board[cell_y][cell_x]
                        prey = cell_y
                        prex = cell_x
                        leg = small_legal_moves(board, player)
                        legals = [result for result in leg if (cell_y, cell_x) in result]
                        legal = [result[1] for result in legals]
                        if prev == player:
                            board[cell_y][cell_x] = 'sel'
                        elif prev == player + "K":
